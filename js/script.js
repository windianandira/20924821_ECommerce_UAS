...
function tambahKeKeranjangDetail(event) {
  event.stopPropagation();

  const container = event.target.closest(".produk-detail");
  const nama = container.querySelector("h3").textContent;
  const harga = container.querySelector(".harga").textContent;
